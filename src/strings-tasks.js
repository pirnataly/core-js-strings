...
function removeFirstOccurrences(str, value) {
  if (str.indexOf(value) !== -1) {
    ...
    const indexEnd = str.indexOf(value) + value.length;
...
function removeLastOccurrences(str, value) {
  if (str.lastIndexOf(value) !== -1) {
    ...
    const indexEnd = str.lastIndexOf(value) + value.length;
    return str.slice(0, indexStart) + str.slice(indexEnd);
  }
  return str;
}
...
function sumOfCodes(str) {
  if (typeof str === 'string') {
    ...
    return strArray.reduce(
      (acc, currentValue) => acc + currentValue.charCodeAt(0),
      0
    );
  }
  return Number(!!str);
...
function countVowels(str) {
  ...
  const result = arr.reduce(
    (acc, char) => acc + Vowels.filter((vowel) => vowel === char).length,
    0
  );
  return result;
}
...
function isPalindrome(str) {
  ...
  newstr = newstr.replace(/ /g, '').replace(/,/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  const newsentence = sentence.split(' ').sort(compare);
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((current) => current.split('').reverse().join(''))
    .join(' ');
}
